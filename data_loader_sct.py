...
def loadCSVfile(csv_name):
    tmp = np.loadtxt(csv_name, dtype=np.str, delimiter=",")
    data = tmp[0:, 0].astype(np.int)  # 加载数据部分
    label = tmp[0:, 1].astype(np.int)  # 加载类别标签部分
    # print(data)
    # print(label)
    return data, label
...
TEXT2TEXT_PATH = './datasets/TextToTextCategorization/'
TEXT2IMAGE_PATH = './datasets/TextToImageClassification/'

# image and image transfer
DATASETS = {
    ...
    'nustag': osp.join(TEXT2IMAGE_PATH, 'nustag_hist.mat'),
    'imagenet': osp.join(TEXT2IMAGE_PATH, 'imagenet_decaf.mat'),
    'english': osp.join(TEXT2TEXT_PATH, 'english_tfidf.mat'),
    'french': osp.join(TEXT2TEXT_PATH, 'french_tfidf.mat'),
    'german': osp.join(TEXT2TEXT_PATH, 'german_tfidf.mat'),
    'italian': osp.join(TEXT2TEXT_PATH, 'italian_tfidf.mat'),
    'spanish5': osp.join(TEXT2TEXT_PATH, 'spanish_tfidf.mat'),
    'spanish10': osp.join(TEXT2TEXT_PATH, 'spanish_tfidf.mat'),
    'spanish15': osp.join(TEXT2TEXT_PATH, 'spanish_tfidf.mat'),
    'spanish20': osp.join(TEXT2TEXT_PATH, 'spanish_tfidf.mat')
}


def get_configuration(args, is_train=False):
    ...
    for cls in range(class_number):
        ...
        while amount > 0:
            ...
            # print('random_index_len', entire_t.shape[0])
            # print('random_index_type', type(random_index))
            if entire_t_label[random_index] == cls:
                # print('select', random_index, cls)
                data_select = [random_index, cls]
